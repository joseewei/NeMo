# =============================================================================
# Copyright (c) 2020 NVIDIA. All Rights Reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
# =============================================================================

from dataclasses import dataclass

import pytorch_lightning as ptl
from omegaconf import DictConfig

from nemo.core.config import Config, TrainerConfig, set_config
from nemo.utils import logging

from typing import Dict, Optional

from torch.optim import Optimizer
from torch.utils.data import DataLoader

from nemo.collections.cv.datasets import MNISTDataset, MNISTDatasetConfig
from nemo.collections.cv.losses import NLLLoss
from nemo.collections.cv.modules import LeNet5 as LeNet5Module
from nemo.core.classes import ModelPT
from nemo.core.classes.common import typecheck
from nemo.core.config import Config, DataLoaderConfig, NovogradConfig, AdamConfig
from nemo.core.neural_types import *
from nemo.core.optim.optimizers import get_optimizer
from nemo.utils.decorators import experimental


@dataclass
class EdgeCaseModelConfig(Config):
    """
    Structured config for LeNet-5 model class - that also contains parameters of dataset and dataloader.

    (This example shows that we can inherit from other configs.)

    Args:
        dataset: MNIST dataset config.
        dataloader: Dataloader config.
        module: module config (default one).
        opt: Lets use Novograd optimizer.
    """
    module: Config = Config()
    dataset: MNISTDatasetConfig = MNISTDatasetConfig(width=32, height=32)
    dataloader: DataLoaderConfig = DataLoaderConfig(batch_size=64, shuffle=True)
    opt1: NovogradConfig = NovogradConfig()
    opt2: AdamConfig = AdamConfig()


@experimental
class EdgeCaseModel(ModelPT):
    def __init__(self, cfg: EdgeCaseModelConfig = EdgeCaseModelConfig()):
        super().__init__()
        # Remember config - should be moved to base init.
        self._cfg = cfg

        # Initialize modules.
        self.module = LeNet5Module(cfg.module)
        self.loss = NLLLoss()

        # Create optimizer 1.
        optimizer1_cls = get_optimizer(self._cfg.opt1.cls)
        self._optimizer1 = optimizer1_cls(params=self.parameters(), **self._cfg.opt1.params)

        # Create optimizer 2.
        optimizer2_cls = get_optimizer(self._cfg.opt2.cls)
        self._optimizer2 = optimizer2_cls(params=self.parameters(), **self._cfg.opt2.params)

    @property
    def input_types(self) -> Optional[Dict[str, NeuralType]]:
        return self.module.input_types

    @property
    def output_types(self) -> Optional[Dict[str, NeuralType]]:
        return self.module.output_types

    @typecheck()
    def forward(self, images):
        return self.module.forward(images=images)

    def set_optimizer1(self):
        self._used_optimizer = self._optimizer1

    def set_optimizer2(self):
        self._used_optimizer = self._optimizer1

    def configure_optimizers(self):
        return self._used_optimizer

    def training_step(self, batch, what_is_this_input):
        # "Unpack" the batch.
        _, images, targets, _ = batch

        # Get predictions.
        predictions = self(images=images)

        # Calculate loss.
        loss = self.loss(predictions=predictions, targets=targets)

        # Return it.
        return {"loss": loss}
        # of course "return loss" doesn't work :]

    def train_dataloader(self):
        """ Create dataset, wrap it with dataloader and return the latter """
        # Instantiate dataset.
        mnist_ds = MNISTDataset(self._cfg.dataset)
        # Configure data loader.
        train_dataloader = DataLoader(dataset=mnist_ds, **(self._cfg.dataloader))

        return train_dataloader

    def setup_training_data(self, train_data_layer_params: Optional[Dict]):
        pass

    def setup_validation_data(self, val_data_layer_params: Optional[Dict]):
        pass

    def setup_test_data(self, test_data_layer_params: Optional[Dict]):
        pass

    def setup_optimization(self, optim_params: Optional[Dict] = None) -> Optimizer:
        pass

    def save_to(self, save_path: str):
        pass

    @classmethod
    def restore_from(cls, restore_path: str):
        pass

    @classmethod
    def list_available_models(cls) -> Optional[Dict[str, str]]:
        pass

    @classmethod
    def from_pretrained(cls, name: str):
        pass

    def export(self, **kwargs):
        pass



@dataclass
class AppConfig(Config):
    """
    This is structured config for this application.

    Args:
        name: Description of the application.
        trainer: configuration of the trainer.
        model: configuation of the model.
    """

    name: str = "Training of a LeNet-5 Model using a pure PyTorchLightning approach - using DDP on 2 GPUs."
    trainer: TrainerConfig = TrainerConfig(gpus=2, distributed_backend="dp")
    model: EdgeCaseModelConfig = EdgeCaseModelConfig()
    max_steps_phase1: int=10
    max_steps_phase2: int=5
    


@set_config(config=AppConfig)
def main(cfg: DictConfig):
    # Show configuration - user can modify every parameter from command line!
    logging.info("=" * 40 + " Hydra says hello! " + "=" * 40 + "\n" + cfg.pretty())

    # The "model" - with dataloader/dataset inside of it.
    model = EdgeCaseModel(cfg.model)

    logging.info("First phase of training - use first optimizer")
    model.set_optimizer1()
    trainer1 = ptl.Trainer(**(cfg.trainer), max_steps=cfg.max_steps_phase1)
    trainer1.fit(model=model)

    logging.info("First phase of training - use second optimizer")
    model.set_optimizer2()
    trainer2 = ptl.Trainer(**(cfg.trainer), max_steps=cfg.max_steps_phase2)
    trainer2.fit(model=model)


if __name__ == "__main__":
    main()
