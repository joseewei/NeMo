...
__all__ = ['MaskPredictTransformerMTModel']


class MaskPredictTransformerMTModel(ModelPT):
    """
    Mask Predict Transformer language model.
    """
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.length_predictor = nn.Linear(
            cfg.machine_translation.hidden_size, cfg.machine_translation.max_seq_length
        )
        self.beam_search = NonAutoregressiveSequenceGenerator(
        ...
        self.len_loss_fn = nn.CrossEntropyLoss()
    ...
    @typecheck()
    def forward(self, src, src_mask, tgt, tgt_mask, tgt_lens):
        """
        ...
        len_preds = self.length_predictor(src_hiddens[:, 0])
        len_loss = self.len_loss_fn(len_preds, tgt_lens)
        tgt_embeddings = self.tgt_embedding_layer(input_ids=tgt)
        # tgt_embeddings *= tgt_embeddings.new_tensor(self.emb_scale)
        tgt_hiddens = self.decoder(
            tgt_embeddings, tgt_mask, src_hiddens,
            src_mask, non_autoregressive=True
        )
        ...
        return log_probs, beam_results, len_loss
    ...
    def training_step(self, batch, batch_idx):
        ...
        src_ids, src_mask, tgt_ids, tgt_mask, labels, _, tgt_lens = batch
        log_probs, _, len_loss = self(src_ids, src_mask, tgt_ids, tgt_mask, tgt_lens)
        ...
        return {'loss': train_loss + len_loss, 'log': tensorboard_logs}
    ...
    def eval_step(self, batch, batch_idx, mode):
        for i in range(len(batch)):
            if batch[i].ndim == 3:
                ...
                # is excess.
                batch[i] = batch[i].squeeze(dim=0)
        src_ids, src_mask, tgt_ids, tgt_mask, labels, sent_ids, tgt_lens = batch
        log_probs, beam_results, len_loss = self(src_ids, src_mask, tgt_ids, tgt_mask, tgt_lens)
