...
# https://github.com/IBM/pytorchpipe/blob/develop/ptp/components/statistics/precision_recall_statistics.py
...
__all__ = ['PrecisionRecallMetric']


class PrecisionRecallMetric(NonTrainableNM):
    """
    Class collecting the Precision/Recall/F1 metric.

    """

    def __init__(self, ix_to_word_mappings: Optional[Dict(int, str)] = None, name: Optional[str] = None):
        """
        Initializes the object.

        Args:
            ix_to_word_mappings: index to word mappings (DEFAULD: none)
    ...
    def input_data_definitions(self):
        ...
        """
        input_defs = {
            self.key_targets: DataDefinition(
                [-1], [torch.Tensor], "Batch of targets, each being a single index [BATCH_SIZE]"
            )
        }

        if self.use_prediction_distributions:
            input_defs[self.key_predictions] = DataDefinition(
                [-1, -1],
                [torch.Tensor],
