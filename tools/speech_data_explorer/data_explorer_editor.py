...
samples_layout = (
    [
        ...
        dbc.Row(
            dbc.Col(
                html.Button(
                    'Export', id='export_button', hidden=True, n_clicks=0, style={'width': '100%', 'height': '100%'}
                )
            ),
            className='mt-3',
        ),
    ...
    + [
        dbc.Row(
            [
                dbc.Col(
                    html.Div(id='dummy', children=('text')),
                    width=2,
                    className='mt-1 bg-light text-monospace text-break small rounded border',
                ),
                dbc.Col(
                    html.Div(dcc.Textarea(id='_text', spellCheck=True, style={'width': '100%'})),
                    className='mt-1 bg-light text-monospace text-break small rounded border',
                ),
                dbc.Col(
                    html.Button(
                        'Update', id='updatetext_button', n_clicks=0, style={'width': '100%', 'height': '100%'}
                    ),
                    width=1,
                    className='mt-1 bg-light text-monospace text-break small rounded border',
                ),
...
@app.callback(
    Output('export_button', 'hidden'), [Input('updatetext_button', 'n_clicks'), Input('export_button', 'n_clicks')],
)
def process_export_button(update_clicks, export_clicks):
    if dash.callback_context.triggered[0]['prop_id'] == 'updatetext_button.n_clicks':
        if update_clicks == 0:
            raise PreventUpdate
        return False
    else:
        if export_clicks == 0:
            raise PreventUpdate
        UPDATES_DIR = os.path.splitext(args.manifest)[0]
        os.makedirs(UPDATES_DIR, exist_ok=True)
        json_filename = os.path.join(UPDATES_DIR, datetime.datetime.now().strftime('%Y%m%d_%H%M') + '.json')
...
@app.callback(
    ...
    [
        ...
        Input('updatetext_button', 'n_clicks'),
    ],
    [
        State('datatable', 'data'),
        State('datatable', 'page_count'),
        State('datatable', 'selected_rows'),
        State('_text', 'value'),
    ],
)
def update_datatable(page_current, sort_by, filter_query, n_clicks, data_state, page_count_state, idx_state, text):
    if dash.callback_context.triggered[0]['prop_id'] == 'updatetext_button.n_clicks' and n_clicks > 0:
        # print(text)
        # print(data_state)
        global data
        data[data_state[idx_state[0]]['index']]['text'] = text
        data_state[idx_state[0]]['text'] = text
        return [data_state, page_count_state]
    data_view = data
    filtering_expressions = filter_query.split(' && ')
    for filter_part in filtering_expressions:
        col_name, op, filter_value = split_filter_part(filter_part)
...
@app.callback(
    [Output('_' + k, 'children') for k in data[0] if k != 'text'] + [Output('_text', 'value')],
    [Input('datatable', 'selected_rows'), Input('datatable', 'data')],
)
def show_item(idx, data):
    if len(idx) == 0:
        raise PreventUpdate
    return [data[idx[0]][k] for k in data[0] if k != 'text'] + [data[idx[0]]['text']]
...
def plot_signal(idx, data):
    if len(idx) == 0:
        raise PreventUpdate
    figs = make_subplots(rows=2, cols=1, subplot_titles=('Spectrogram', 'Waveform'))
    try:
        ...
        figs.update_layout({'margin': dict(l=0, r=0, t=20, b=0, pad=0), 'height': 1400, 'width': 2000})
